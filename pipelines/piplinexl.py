...
class SGA_XL(AttentionBase):
    ...
    def __init__(
        ...
        self.mask_16 = F.max_pool2d(M, (1024 // 16, 1024 // 16)).round().squeeze().squeeze()
        self.mask_32 = F.max_pool2d(M, (1024 // 32, 1024 // 32)).round().squeeze().squeeze()
        self.mask_64 = F.max_pool2d(M, (1024 // 64, 1024 // 64)).round().squeeze().squeeze()
        self.mask_128 = F.max_pool2d(M, (1024 // 128, 1024 // 128)).round().squeeze().squeeze()
        self.markov_map_16 = F.max_pool2d(markov_map, (1024 // 16, 1024 // 16),
                                          ceil_mode=True).round().squeeze().squeeze()
        self.markov_map_32 = F.max_pool2d(markov_map, (1024 // 32, 1024 // 32),
                                          ceil_mode=True).round().squeeze().squeeze()
        self.markov_map_64 = F.max_pool2d(markov_map, (1024 // 64, 1024 // 64),
                                          ceil_mode=True).round().squeeze().squeeze()
        self.markov_map_128 = F.max_pool2d(markov_map, (1024 // 128, 1024 // 128),
                                           ceil_mode=True).round().squeeze().squeeze()
...
def mask_pil_to_torch(M, height, width):
    # preprocess M
    if isinstance(M, (PIL.Image.Image, np.ndarray)):
        M = [M]

    if isinstance(M, list) and isinstance(M[0], PIL.Image.Image):
        M = [i.resize((width, height), resample=PIL.Image.LANCZOS) for i in M]
        M = np.concatenate([np.array(m.convert("L"))[None, None, :] for m in M], axis=0)
        M = M.astype(np.float32) / 255.0
    elif isinstance(M, list) and isinstance(M[0], np.ndarray):
        M = np.concatenate([m[None, None, :] for m in M], axis=0)
...
def preprocess_mask_and_markov(mask_image, markov_original, soft_mask, height=1024, width=1024, points=None,
...
class StableDiffusionXL_AE_Pipeline(
    ...
    def do_self_attention_redirection_guidance(self):  # SARG
        return self._rm_guidance_scale > 1 and self._SGA
    ...
    def __call__(
            ...
            SGA: bool = True,  # CR parameter
            rm_guidance_scale: float = 7.0,  # CR parameter
            sg_steps: int = 9,  # CR parameter
            sg_scale: float = 0.3,  # CR parameter
            SGA_start_step: int = 0,  # CR parameter
            SGA_start_layer: int = 34,  # CR parameter
            SGA_end_layer: int = 70,  # CR parameter
        ...
        self._SGA = SGA
        self._sg_steps = sg_steps
        self._sg_scale = sg_scale
        self._SGA_start_step = SGA_start_step
        self._SGA_start_layer = SGA_start_layer
        self._SGA_end_layer = SGA_end_layer
        ###########
        ...
        # apply SGA to modify the attention module
        if self.do_self_attention_redirection_guidance:
            self._SGA_end_step = int(strength * self._num_timesteps)
            layer_idx = list(range(self._SGA_start_layer, self._SGA_end_layer))
            editor = SGA_XL(
                self._SGA_start_step,
                self._SGA_end_step,
                self._SGA_start_layer,
                self._SGA_end_layer,
                layer_idx=layer_idx,
                M=mask_image,
                markov_map=markov_original,
                model_type="SDXL",
                sg_steps=self._sg_steps,
                sg_scale=self._sg_scale,
            )
